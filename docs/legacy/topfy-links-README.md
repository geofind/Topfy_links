> **Nota de contexto (2026-07-13):** este documento é o README original do projeto "Topfy Links", preservado aqui como referência histórica/de visão após este repositório (`github.com/geofind/Topfy_links`) ter sido adotado como o repositório do **CanalTopfy OS** (ver [`PROJECT_CHARTER.md`](../../PROJECT_CHARTER.md)). Na prática, apenas um scaffold vazio (Vite/React/shadcn gerado pela ferramenta Manus) e um `requirements.txt` de 2 linhas estavam de fato commitados — o pipeline Python/GitHub Actions/Google Drive descrito abaixo é a visão original, não código funcionando neste repo. Mapeamento para a nova arquitetura: a coleta multi-plataforma (Mercado Livre/Amazon) descrita aqui é exatamente o escopo do **ScoutWorker** (Fase 2 — Radar, ver `docs/ARCHITECTURE.md`) — avaliar reaproveitamento ao chegar nessa fase.
